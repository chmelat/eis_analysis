...
| Relativní stderr | > 200% | Velmi vysoká nejistota parametru |
| Numerický rank | < p | Rank-deficientní |

### 5.3 SVD-based výpočet kovariance

```python
# Sloupcové škálování (van der Sluis): jednotky parametrů (R ~ 1e5 Ohm
# vs Q ~ 1e-6) by jinak nafoukly cond(J) čistě ze škál, ne z korelace
col_scale = np.linalg.norm(jacobian, axis=0)
J_scaled = jacobian / col_scale

# SVD dekompozice: J_s = U @ S @ V^T
U, S, Vt = np.linalg.svd(J_scaled, full_matrices=False)

# Condition number normální matice J_s^T J_s (škálově invariantní)
condition_number = (S[0] / S[-1]) ** 2

# Plný rank: přesná inverze, přeškálovaná zpět do původních jednotek
# (J^T J)^{-1} = D^{-1} (J_s^T J_s)^{-1} D^{-1},  D = diag(col_scale)
JsTJs_inv = Vt.T @ np.diag(1/(S*S)) @ Vt
JtJ_inv = JsTJs_inv / np.outer(col_scale, col_scale)

# Kovariance
cov = residual_variance * JtJ_inv
```

**Rank-deficientní případ** (některé singulární hodnoty numericky nulové):
kovariance v postižených směrech neexistuje. Parametry, jejichž směr má
významný překryv s nulovým prostorem (> 10⁻³ normy směru), dostanou
stderr = inf (neidentifikovatelné, warning je jmenuje); ostatní parametry
dostanou variance z Moore-Penroseovy pseudoinverze přes identifikovatelný
podprostor.

**Výhody SVD přístupu:**
- Numericky stabilní i pro ill-conditioned problémy
- Rank a kondice na škálovaném Jacobiánu jsou škálově invariantní
  (jednotky parametrů je neovlivní)
- Detekce rank-deficientních problémů po jednotlivých parametrech

### 5.4 Confidence intervals

Používá t-distribuci s dof = 2·n_freq − n_free stupni volnosti (N frekvencí
dává 2N reálných reziduí: reálná + imaginární část; n_free = volné
parametry — stejné dof jako odhad s²):

```python
t_critical = t.ppf(1 - alpha/2, dof)

# Lineární parametry (exponent n):
CI = params ± t_critical * stderr

# Kladné škálové parametry (R, C, Q, L, σ, τ): delta metoda na ln(p),
# aby CI nemohl protnout nulu (záporný odpor):
f = exp(t_critical * stderr / params)
CI = (params / f, params * f)
```

Škálový parametr = kladná dolní mez a meze přes > 6 dekád (stejné
kritérium jako u detekce parametru na mezi).
...
| sqrt | 1/√\|Z\| | Kompromis |
| modulus (default) | 1/\|Z\| | Všechny frekvence stejně (relativní chyby) |
| proportional | 1/\|Z\|² | Silný důraz na malé \|Z\| (vysoké frekvence) |
...
**Modulus vážení (default):**
- Všechny frekvence přispívají podobně (minimalizují se relativní chyby)
...
| Covariance | SVD-based, sloupcově škálované | Robustní |
