...
**Stav:** všech 11 nálezů aplikováno 2026-07-25 (mechanická skupina
#1, #3, #8-#11 ve `ff6864a`/v0.19.0, zbytek #2, #4-#7 ve v0.20.0).
...
| 4 | delete | **[APLIKOVÁNO 2026-07-25]** Pole `RinfEstimate.R_ct` / `C_nF` / `f_characteristic` jsou vždy `None` — `estimate_rinf_with_inductance()` staví diagnostický dict z explicitního seznamu polí, který tyto klíče vůbec nemá. Obě podmíněné výpisové větve jsou tím nedosažitelné | nic; zrušit 3 pole a obě větve | `drt/results.py:28-30`, `cli/handlers/drt.py:58-61`, `cli/handlers/rinf.py:67-73` |
| 5 | yagni | **[APLIKOVÁNO 2026-07-25]** 20řádkový diagnostický dict znovu poskládaný z `RLKFitResult` "for backward compatibility", včetně aliasu `r_squared`/`R_squared`. Třetí prvek návratové pětice je dokumentován jako "Always None (legacy placeholder)" a všichni tři volající ho zahazují | vracet `(result, fig)`; volající čtou atributy dataclassy | `rinf_estimation/rlk_fit.py:381-420` |
| 6 | shrink | **[APLIKOVÁNO 2026-07-25]** `utils/compat.py` — 32řádkový modul, z toho 26 řádků docstring, kolem 4řádkového import-shimu s jediným volajícím | try/except přímo v `auto_suggest.py:15`; smazat modul a dva exportní řádky v `utils/__init__.py` | `eis_analysis/utils/compat.py` |
| 7 | delete | **[APLIKOVÁNO 2026-07-25]** Parametr `verbose` na 4 veřejných vstupních bodech fittingu, dokumentovaný jako "Ignored". Čtyři volající ho stále zbytečně předávají | nic; úroveň logování je skutečný ovládací prvek | `fitting/circuit.py:254`, `fitting/diffevo.py:156`, `fitting/multistart.py:166`, `rinf_estimation/rlk_fit.py:311` |
...
**Mění veřejné signatury** (#2, #4, #5, #6, #7) — `use_voigt_fit`, `verbose`,
návratová pětice `estimate_rinf_with_inductance()`, `utils/compat.py`.
Aplikováno ve v0.20.0; #6 v původním rozdělení chybělo, patří sem, protože
ruší export `np_trapz`.
