...
| `DQ(A, n, tau_min, U)` | Truncated CPE (bounded power-law DRT) | `DQ(1.2e6, 0.57, 5e-2, 8)` |
...
- `--circuit`, `-c` - Equivalent circuit for fitting. Syntax: `-` = series, `|` = parallel. Example: `"R(100) - (R(5000) | C(1e-6))"`. Supported elements: R, C, L, G, Q, W, Wo, K, GE, CC, DQ. Repeat the option to fit several candidates on the same data and rank them by AIC/BIC - see [Comparing candidate circuits](#comparing-candidate-circuits).
